from abc import ABC, abstractmethod
import numpy as np


# Clase base abstracta

class Particle(ABC):
    """
    Clase abstracta que representa una partícula genérica.
    Todas las partículas tendrán masa, carga, posición, velocidad y aceleración.
    """

    def __init__(self, mass=0.0, charge=0.0,
                 position=None, velocity=None, acceleration=None):
        self.mass = mass
        self.charge = charge
        self.position = np.array(position if position is not None else [0.0, 0.0])
        self.velocity = np.array(velocity if velocity is not None else [0.0, 0.0])
        self.acceleration = np.array(acceleration if acceleration is not None else [0.0, 0.0])

    # Métodos abstractos que deberán implementar las subclases
    @abstractmethod
    def move(self, dt):
        """Actualiza la posición y velocidad según dt"""
        pass

    @abstractmethod
    def setPosition(self, pos):
        pass

    @abstractmethod
    def getPosition(self):
        pass

    @abstractmethod
    def setVelocity(self, vel):
        pass

    @abstractmethod
    def getVelocity(self):
        pass

    @abstractmethod
    def setAcceleration(self, acel):
        pass

    @abstractmethod
    def getAcceleration(self):
        pass



# Clase Electron

class Electron(Particle):
    def __init__(self):
        super().__init__(mass=9.11e-31, charge=-1.60e-19)

    def setPosition(self, pos):
        self.position = np.array(pos)

    def getPosition(self):
        return self.position

    def setVelocity(self, vel):
        self.velocity = np.array(vel)

    def getVelocity(self):
        return self.velocity

    def setAcceleration(self, acel):
        self.acceleration = np.array(acel)

    def getAcceleration(self):
        return self.acceleration

    def move(self, dt):
        # Movimiento simple
        self.velocity += self.acceleration * dt
        self.position += self.velocity * dt

    def emisionFoton(self, energia):
        print(f"El electrón emite un fotón de energía {energia} J")

    def calDeBroglieLongitudOnda(self):
        # λ = h / (m * v)
        h = 6.62607015e-34
        v = np.linalg.norm(self.velocity)
        if v == 0:
            return np.inf
        return h / (self.mass * v)



# Clase Proton

class Proton(Particle):
    def __init__(self):
        super().__init__(mass=1.6726e-27, charge=1.60e-19)

    def setPosition(self, pos):
        self.position = np.array(pos)

    def getPosition(self):
        return self.position

    def setVelocity(self, vel):
        self.velocity = np.array(vel)

    def getVelocity(self):
        return self.velocity

    def setAcceleration(self, acel):
        self.acceleration = np.array(acel)

    def getAcceleration(self):
        return self.acceleration

    def move(self, dt):
        self.velocity += self.acceleration * dt
        self.position += self.velocity * dt

    def calcularMomentoMagnetico(self):
        magneton_nuclear = 5.050783699e-27
        g_factor = 5.5856946893
        return g_factor * magneton_nuclear


# Clase Neutron

class Neutron(Particle):
    def __init__(self):
        super().__init__(mass=1.6749e-27, charge=0.0)

    def setPosition(self, pos):
        self.position = np.array(pos)

    def getPosition(self):
        return self.position

    def setVelocity(self, vel):
        self.velocity = np.array(vel)

    def getVelocity(self):
        return self.velocity

    def setAcceleration(self, acel):
        self.acceleration = np.array(acel)

    def getAcceleration(self):
        return self.acceleration

    def move(self, dt):
        self.velocity += self.acceleration * dt
        self.position += self.velocity * dt

    def calMomentoMagnetico(self):
        magneton_nuclear = 5.050783699e-27
        g_factor = -3.82608545
        return g_factor * magneton_nuclear
